#include<iostream>
#include<vector>
#include<algorithm>
using namespace std;

class Edge{
    public:
    int u;
    int v;
    int wt;

    Edge(int u, int v, int wt){
        this->u = u;
        this->v = v;
        this->wt = wt;        
    }   

};

class Graph{
    public: 
    int V; 
    vector<Edge> edges;
    vector<int> par;
    vector<int> rank;
    
    Graph(int V){
        this->V = V;

        for (int i = 0; i < V; i++)
        {
            par.push_back(i);
            rank.push_back(0);
        }
        
    }

    void addEdges(int u, int v, int wt){
        edges.push_back(Edge(u,v,wt));
    }

    int find(int x){
        if(par[x] == x){
            return x;
        }

        return par[x] = find(par[x]);
    } 

    void unionByRank(int u, int v){
        int parU = find(u);
        int parV = find(v);

        if(rank[parU] == rank[parV]){
            par[parV] = parU;
            rank[parU]++;
        }else if(rank[parU] > rank[parV]){
            par[parV] = parU;
        }else{
            par[parU] = parV;
        }
    }

    void kruskals(){ //  TC = 0(ElogE)
        sort(edges.begin(), edges.end(), [](Edge &a, Edge &b){return a.wt < b.wt ;});
        int minCost = 0;
        int count= 0; // used for optimisation

        for(int i = 0; i<edges.size() && count < V-1; i++){
            Edge e = edges[i];

            int parU = find(e.u);
            int parV = find(e.v);
            if(parU != parV){ // if there is no cycle then add cost of that edge to get minm spanning tree
                unionByRank(parU, parV); // to addition of parV to parU
                minCost += e.wt;
                count++;
            }
        }
        cout<<"min cost of MST is "<<minCost<<endl;
    }
};
int main(){ 
    Graph graph(4);

    graph.addEdges(0,1,10);
    graph.addEdges(0,2,15);
    graph.addEdges(0,3,30);
    graph.addEdges(1,3,40);
    graph.addEdges(2,3,50);
    
    graph.kruskals();
    return 0; 
}